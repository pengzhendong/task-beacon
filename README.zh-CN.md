...
菜单栏应用会自动启动随包的本地服务，并把 CLI 工具链接到 `~/.local/bin`。底部的 **安装 CLI** 可用于冲突时重试和查看原因。随后可以直接使用：
...
`--command` 会交给 `/bin/zsh` 执行。如果参数恰好是一个已存在的脚本路径，TaskBeacon
会自动为它做 shell 引用，因此路径中包含空格时也不需要额外包装。带参数、管道或重定向的
命令仍按 shell 源码处理，需要遵循正常的 shell 引用规则。
...
仅为防止轮询重叠时，采集脚本无需再自行加锁。如果采集器因其他原因必须通过外部锁协调，
应让锁具备陈旧恢复能力：超时可能在 cleanup trap 执行前终止 shell，使普通锁文件或锁目录残留。
